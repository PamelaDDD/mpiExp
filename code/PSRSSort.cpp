...
int main(int argc,char* argv[]) {
    ...
    if (dataLength == pow(2,32)){
        dataLength = dataLength - 4096;
    }
    ...
    printf("mydatalength:%ld\n",myDataLength);
    // 3. 各进程获取数据
    float *myData = new float[myDataLength];
    printf("new array success!\n");
    ...
    printf("regularsamples new sucess!\n");
    ...
    float *gatherRegSam;
    if(my_rank == 0)
        if (comm_sz * comm_sz < 0)
            printf("invali comm_sz!\n");
    ...
    printf("recvRankPartLen new success!\n");
    ...
    printf("rankPartStart new success!\n");
    for(int i=0; i<comm_sz; i++) {
    ...
    printf("recvPartData new success!\n");
    ...
    if(my_rank == 0) {
        ...
        cout << "processors:"<<comm_sz << "power:"<<endl;
